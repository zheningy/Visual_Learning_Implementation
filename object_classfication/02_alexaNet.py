...
def cnn_model_fn(features, labels, mode, num_classes=20):
    ...
    ix = 13
    iy = 13
    conv1_kernel = tf.reshape(conv1_kernel, (11, 11, 3 * 96))
    conv1_kernel = tf.image.resize_image_with_crop_or_pad(conv1_kernel, iy, ix)
    cx = 12
    cy = 8
    conv1_kernel = tf.reshape(conv1_kernel, (iy, ix, 3, cx, cy))
    weight_images = tf.transpose(conv1_kernel, (4, 0, 3, 1, 2))
    weight_images = tf.reshape(weight_images, (1, cy*iy, cx*ix, 3))
    tf.summary.image("weight_images",weight_images)

    # for i in range(conv1_kernel.shape[3]):
    #     curt_filter = tf.slice(conv1_kernel, [0, 0, 0, i], [11, 11, 3, 1])
    #     filter_image = tf.reshape(curt_filter, [1, 11, 11, 3])
    #     tf.summary.image("conv1_filter_%d"%i, filter_image)
...
def main():
    ...
    pascal_classifier = tf.estimator.Estimator(
        ...
        model_dir="models/pascal_model_alexNet_test")
