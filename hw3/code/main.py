...
def get_data(file, shuffle=True):
...
def main():
    ...
    trainval_data = get_data('annotated_train_set.p')
    ...
    log = Logger(os.path.join(os.getcwd(), 'log'), 'simple_net')
...
def train(train_loader, model, criterion, optimizer, epoch, log):
    ...
    for i, (features, target) in enumerate(train_loader):
        ...
        #     curt_frame = torch.autograd.Variable(features[i], requires_grad=True)
        #     output = model(curt_frame)
